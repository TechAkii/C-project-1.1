...
int main() {
    int reg, j, i = 0, found = 0, choice, cartcount = 0, quantity;
    char itemName[50];
    ...
    if (reg == 0) {
        ...
        while (attempts < 4) {
            ...
            if (found) {
                ...
                attempts++;
                printf("Login failed! Incorrect email or password. You have %d attempt(s) left.\n", 4 - attempts);
            }
        }

        fclose(filePtr);
        printf("Too many failed attempts. Login process canceled.\n");
    } else {
        printf("Invalid input. Please enter 1 for login or 0 for registration.\n");
    }
